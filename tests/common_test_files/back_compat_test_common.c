...
static void ReceiveConfirmationCallback(IOTHUB_CLIENT_CONFIRMATION_RESULT result, void* user_ctx)
{
    LogInfo("ReceiveConfirmationCallback invoked, result=<%s>, user_ctx=<%p>", ENUM_TO_STRING(IOTHUB_CLIENT_CONFIRMATION_RESULT, result), user_ctx);

    BACK_COMPAT_INFO* back_compat_info = (BACK_COMPAT_INFO*)user_ctx;
...
{
    LogInfo("connection_status_callback invoked, status=<%s>, user_ctx=<%p>", ENUM_TO_STRING(IOTHUB_CLIENT_CONNECTION_STATUS, status), user_ctx);
    BACK_COMPAT_INFO* back_compat_info = (BACK_COMPAT_INFO*)user_ctx;
    if (back_compat_info != NULL)
    {
...
static int parse_connection_string(const IOTHUB_PROVISIONED_DEVICE* device_info, IOTHUB_CLIENT_CONFIG* client_config)
{
    ...
    while (iterator != NULL && *iterator != NULL)
    {
        if (strcmp(iterator, "HostName=") == 0)
        {
            ...
            while (*iterator != NULL)
            {
                if (*iterator == '.')
                {
                    ...
                    {
                        client_config->iotHubName = (const char*)malloc(counter+1);
                        if (client_config->iotHubName == NULL)
                        {
                            result = __FAILURE__;
                        }
                        ...
                        {
                            strncpy(client_config->iotHubName, )
                        }
                    }
                }
                else if (*iterator == ';')
        ...
        else if (strcmp(iterator, "SharedAccessKey=") == 0)
        {
        }
        iterator++;
    }
...
static IOTHUB_DEVICE_CLIENT_LL_HANDLE create_device_client(DEVICE_CREATION_TYPE type, IOTHUB_CLIENT_TRANSPORT_PROVIDER protocol)
{
    IOTHUB_DEVICE_CLIENT_LL_HANDLE result;

    IOTHUB_PROVISIONED_DEVICE* device_info = IoTHubAccount_GetSASDevice(g_iothubAcctInfo);
    ...
    switch (type)
    {
        ...
        case DEVICE_CREATION_CREATE:
        {
            ...
            parse_connection_string(device_info, &config);
            config.deviceId = device_info->deviceId;
            config.deviceKey = device_info->primaryAuthentication;
            config.protocol = protocol;
            result = IoTHubDeviceClient_LL_Create(&config);
            break;
        }
        case DEVICE_CREATION_WITH_TRANSPORT:
        {
            IOTHUB_CLIENT_DEVICE_CONFIG config;
            result = IoTHubDeviceClient_LL_CreateWithTransport(&config);
        }
        default:
            result = NULL;
            break;
...
static int test_all_device_options(TEST_PROTOCOL_TYPE protocol_type, IOTHUB_DEVICE_CLIENT_LL_HANDLE device_client)
{
    ...
    bool trace_on = true;
    IoTHubDeviceClient_LL_SetOption(device_client, OPTION_LOG_TRACE, &trace_on);
    IoTHubDeviceClient_LL_SetOption(device_client, OPTION_KEEP_ALIVE, &trace_on);
    IoTHubDeviceClient_LL_SetOption(device_client, OPTION_CONNECTION_TIMEOUT, &trace_on);
    IoTHubDeviceClient_LL_SetOption(device_client, OPTION_SAS_TOKEN_LIFETIME, &trace_on);
    IoTHubDeviceClient_LL_SetOption(device_client, OPTION_SAS_TOKEN_REFRESH_TIME, &trace_on);
    IoTHubDeviceClient_LL_SetOption(device_client, OPTION_MESSAGE_TIMEOUT, &trace_on);
    IoTHubDeviceClient_LL_SetOption(device_client, OPTION_PRODUCT_INFO, "custom_product_info");

    // Proxy host???

    // MQTT only
    IoTHubDeviceClient_LL_SetOption(device_client, OPTION_AUTO_URL_ENCODE_DECODE, &trace_on);

    // Amqp only
    IoTHubDeviceClient_LL_SetOption(device_client, OPTION_CBS_REQUEST_TIMEOUT, &trace_on);
    IoTHubDeviceClient_LL_SetOption(device_client, OPTION_MIN_POLLING_TIME, &trace_on);
    IoTHubDeviceClient_LL_SetOption(device_client, OPTION_BATCHING, &trace_on);
    IoTHubDeviceClient_LL_SetOption(device_client, OPTION_SERVICE_SIDE_KEEP_ALIVE_FREQ_SECS, &trace_on);
    IoTHubDeviceClient_LL_SetOption(device_client, OPTION_C2D_KEEP_ALIVE_FREQ_SECS, &trace_on);
    IoTHubDeviceClient_LL_SetOption(device_client, OPTION_REMOTE_IDLE_TIMEOUT_RATIO, &trace_on);
    IoTHubDeviceClient_LL_SetOption(device_client, OPTION_EVENT_SEND_TIMEOUT_SECS, &trace_on);
...
static int send_telemetry_message(IOTHUB_DEVICE_CLIENT_LL_HANDLE device_client, MESSAGE_CREATION_MECHANISM msg_type, void* context)
{
    ...
    else
    {
        ...
        if (IoTHubDeviceClient_LL_SendEventAsync(device_client, event_msg, ReceiveConfirmationCallback, context) != IOTHUB_CLIENT_OK)
        {
...
int back_compat_init(TEST_PROTOCOL_TYPE protocol_type)
{
    ...
    {
        if ((g_iothubAcctInfo = IoTHubAccount_Init(false)) == NULL)
        {
...
int send_telemetry_with_device_connection_string(DEVICE_CREATION_TYPE create_type, MESSAGE_CREATION_MECHANISM msg_type, IOTHUB_CLIENT_TRANSPORT_PROVIDER protocol)
{
    int result;
    BACK_COMPAT_INFO back_info = { 0 };
    IOTHUB_DEVICE_CLIENT_LL_HANDLE device_client = create_device_client(create_type, protocol);
    ...
    else
    {
        if (device_client != NULL)
        {
            IoTHubDeviceClient_LL_SetConnectionStatusCallback(device_client, connection_status_callback, &back_info);
        }

        if (send_telemetry_message(device_client, msg_type, &back_info) != 0)
        {
            result = __LINE__;
        }
        else
